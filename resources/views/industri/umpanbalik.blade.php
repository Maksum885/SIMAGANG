...
@section('content')
<div class="text-md">
    <h2 class="text-2xl underline font-bold mb-5">Umpan Balik untuk Siswa/i</h2>
    {{-- Alert Messages --}}
    @if(session('success'))
    ...
    <div class="bg-white p-6 rounded shadow w-full mb-5">
        <h3 class="text-xl font-bold mb-4">- Kirim Umpan Balik -</h3>
        <form action="{{ route('industri.umpanbalik.store') }}" method="POST">
            @csrf
            {{-- Pilih Siswa --}}
            <div class="mb-2">
                <label for="siswa_id" class="block font-medium text-gray-700 mb-2">Nama Siswa/i</label>
                <select name="siswa_id" id="siswa_id" required class="w-full border rounded px-2 py-1 @error('siswa_id') border-red-500 @enderror">
            ...
            {{-- Area Masukan --}}
            <div class="mb-2">
                <label for="isi_umpan_balik" class="block font-medium text-gray-700 mb-2">Masukan / Umpan Balik</label>
                <textarea name="isi_umpan_balik" id="isi_umpan_balik" rows="6" required
                    class="w-full border rounded px-2 py-2 resize-none @error('isi_umpan_balik') border-red-500 @enderror"
            ...
            <div class="flex justify-end">
                <button type="submit" class="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 cursor-pointer">
                    Kirim Umpan Balik
                </button>
            </div>
        </form>
    ...
    <div class="bg-white p-6 rounded shadow w-full">
        <h3 class="text-xl font-bold mb-4">- Riwayat Umpan Balik -</h3>
                ...
                <div class="flex justify-between items-start mb-2">
                    ...
                    <div class="text-right">
                        ...
                        <a href="{{ route('industri.umpanbalik.edit', $umpanBalik->id) }}"
                            class="text-blue-600 text-lg hover:underline">Edit</a>
                    </div>
                </div>
