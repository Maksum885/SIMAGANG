...
@section('content')
<div>
    <h1 class="text-2xl font-bold mb-5 underline">Kelola Pengguna</h1>
    @if (session('success'))
    ...
    <div>
        <div class="flex justify-between mb-5 text-md">
            <input type="text" id="cariInput" placeholder="Cari..." class="border px-2 py-1 rounded w-1/3">
            <button onclick="showModal('modal-tambah-pengguna')" class="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded cursor-pointer">
        ...
        <table class="w-full bg-white rounded shadow text-left text-lg">
            <thead class="bg-gray-300">
                <tr>
                    <th class="px-4 py-1 text-left">Username</th>
                    <th class="px-4 py-1 text-left">Nama Lengkap</th>
                    <th class="px-4 py-1 text-left">Role</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-300">
                    <td class="px-4 py-1 text-left">{{ $user->username }}</td>
                    <td class="px-4 py-1 text-left">{{ $user->nama_lengkap }}</td>
                    <td class="px-4 py-1 text-left">{{ $user->role }}</td>
                ...
                <form action="{{ route('admin.kelola.store') }}" method="POST">
                    ...
                    <div class="p-2">
                        <div class="mb-2">
                            ...
                            <input type="text" class="w-full border rounded-lg px-2 py-1" name="username" value="{{ old('username') }}" />
                        </div>
                        <div class="mb-2">
                            <label class="block font-medium text-gray-700 mb-1">Nama Lengkap</label>
                            <input type="text" class="w-full border rounded-lg px-2 py-1" name="nama_lengkap" value="{{ old('nama_lengkap') }}" />
                        </div>
                        <div class="mb-2">
                            <label class="block font-medium text-gray-700 mb-1">Role</label>
                            <select class="w-full border rounded-lg px-2 py-1" name="role">
                    ...
                    <div class="p-2 border-t border-gray-200 flex justify-end space-x-2">
                        ...
                        <button
                            type="submit"
                            class="px-2 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 cursor-pointer">
                            Simpan
                        </button>
                    </div>
                </form>
        ...
        <div id="modal-edit-pengguna-{{ $user->id }}" class="hidden fixed inset-0 flex items-center justify-center">
            <div class="bg-white w-1/3 p-5 rounded-lg border-2 shadow-lg relative text-md">
                <h2 class="text-xl font-bold underline mb-1">Edit Pengguna</h2>
                <form method="POST" action="{{ route('admin.kelola.update', $user->id) }}">
                    ...
                    <div class="p-2">
                        <form>
                            <div class="mb-2">
                                <label class="block font-medium text-gray-700 mb-1">Username</label>
                                <input type="text" class="w-full border rounded-lg px-2 py-1" name="username" value="{{ old('username', $user->username) }}" />
                            </div>
                            <div class="mb-2">
                                <label class="block font-medium text-gray-700 mb-1">Nama Lengkap</label>
                                <input type="text" class="w-full border rounded-lg px-2 py-1" name="nama_lengkap" value="{{ old('nama_lengkap', $user->nama_lengkap) }}" />
                            </div>
                            <div class="mb-2">
                                <label class="block font-medium text-gray-700 mb-1">Password Baru (opsional)</label>
                                <input type="text" class="w-full border rounded-lg px-2 py-1" name="password" placeholder="Kosongkan jika tidak diubah" />
                            </div>
                    </div>
                    <div class="p-2 border-t border-gray-200 flex justify-end space-x-2">
                        <button type="button" onclick="hideModal('modal-edit-pengguna-{{ $user->id }}')" class="px-2 py-1 border rounded-lg hover:bg-gray-50 cursor-pointer">Batal</button>
        ...
        <div id="modal-hapus-pengguna-{{ $user->id }}" class="hidden fixed inset-0  flex items-center justify-center z-50">
            <div class="bg-white w-1/3 p-5 border-2 rounded-lg shadow-lg relative text-md">
                <h2 class="text-xl font-bold mb-3 text-center underline">Hapus Penggguna</h2>
                <p class="text-center mb-5">Apakah Anda yakin ingin menghapus {{ $user->nama_lengkap }}?</p>
                <form method="POST" action="{{ route('admin.kelola.destroy', $user->id) }}">
                    @csrf
                    @method('DELETE')
                    <div class="flex justify-center space-x-4">
                        <button type="button" onclick="hideModal('modal-hapus-pengguna-{{ $user->id }}')" class="px-2 py-1 border rounded-lg hover:bg-gray-50 cursor-pointer">Batal</button>
...
<script>
    document.getElementById('cariInput').addEventListener('input', function() {
        ...
        rows.forEach(row => {
            ...
            const namaLengkap = row.cells[1].textContent.toLowerCase();
            const role = row.cells[2].textContent.toLowerCase();

            if (username.includes(filter) || namaLengkap.includes(filter) || role.includes(filter)) {
                row.style.display = '';
            } else {
